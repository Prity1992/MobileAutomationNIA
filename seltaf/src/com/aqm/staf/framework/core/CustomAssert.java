...
public class CustomAssert extends Assert {
	...
	public void assertEquals (String expected, String actual, AssertionType assertType) {
		try { 
			...
			FrameworkServices.logMessage("<B><Font Color = \"white\">"+counter+".  Actual= "+actual+";  Expected= "+expected+" ; PASSED </Font> </B>");
		}
	...
	public void customLogger(String logMessage,boolean resultLog,AssertionType assertType) {
		try {
			counter++;
			assertTrue(resultLog);
		   FrameworkServices.logMessage("<B><Font Color = \"white\">"+counter+".  "+logMessage+" ; PASSED </Font> </B>");
		  }
		catch(AssertionError error) {
			String ErrorMessage;
			ErrorMessage="Excel value"+"-"+ error.getMessage()+"-"+"at Application";
			...
			case WARNING:
				warning = true;
				FrameworkServices.logMessage("<B><Font Color = \"yellow\">"+counter+".  "+logMessage+" ; FAILED </Font> </B>");
				captureScreenShotOnAssertionException(AutomationDriverScript.scenarioExecutionFolderReference,ErrorMessage);
				break;
			}
		}
