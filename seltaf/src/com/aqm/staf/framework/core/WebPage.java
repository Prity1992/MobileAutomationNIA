...
public abstract class WebPage {
	...
	private WebElement waitForElementAndReturnElement(_PageElement pageElement) {
	...
	protected void waitForPageElement(_PageElement pageElement) {
		...
		case WAITFORELEMENTTOBECLICKABLE:
			try {
				...
				throw new ScriptExecutionException (pageElement.getName() + " was not clickable within specified wait time "+ pageElement.getTimeOut(), e,pageName);
			}
			break;
		...
		case WAITFORELEMENTTOBEEENABLED:
			try {
				...
				throw new ScriptExecutionException (pageElement.getName() + " was not loaded within specified wait time "+ pageElement.getTimeOut(), e,pageName);
			}
			break;
		...
		case WAITFORELEMENTTOBEDISPLAYED:
			try {
				...
				throw new ScriptExecutionException (pageElement.getName() + " was not displayed within specified wait time "+ pageElement.getTimeOut(), e,pageName);
			}
			break;
		...
		case WAITFORELEMENTTODISAPPEAR:
			try {
				...
				throw new ScriptExecutionException (pageElement.getName() + " did not disappered within specified wait time "+ pageElement.getTimeOut(), e,pageName);
			}
			break;
		default:
			break;
	...
	private boolean isWebElementAvailableInPageElement(_PageElement pageElement) {
		return !(pageElement.getWebElement() == null);
	}

	protected WebElement getWebElement(_PageElement pageElement) {
	...
	protected void sendKeys (_PageElement pageElement, String value) {
		try {
			...
			throw new ScriptExecutionException ("Failed to type value: '" + value + "' in " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void sendKeys (Keys key, _PageElement pageElement) {
		try {
			if (!isWebElementAvailableInPageElement(pageElement))
			...
			throw new ScriptExecutionException("Failed to press : '" + key + "' in " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void clearAndSendKeys (_PageElement pageElement, String value) {
		try {
			WebElement element;
			if (!isWebElementAvailableInPageElement(pageElement))
			...
			throw new ScriptExecutionException ("Failed to clear and type value: '" + value + "' in " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void SelectValueAndTypeForTextField (_PageElement pageElement, String value) {
		try {
			WebElement element;
			if (!isWebElementAvailableInPageElement(pageElement))
				element = getWebElement(pageElement);
	...
	protected void SelectValueAndTypeForHomeTextField (_PageElement pageElement, String value) {
		try {
			WebElement element;
			if (!isWebElementAvailableInPageElement(pageElement))
				element = getWebElement(pageElement);
	...
	protected void click(_PageElement pageElement) {
		try {
			WebElement element;
			if (!isWebElementAvailableInPageElement(pageElement))
				element = getWebElement(pageElement);
			...
			throw new ScriptExecutionException ("Failed to click on : '" + pageElement.getName() + "' on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void check(_PageElement pageElement) {
		try {
			...
			throw new ScriptExecutionException ("Failed to check: '" + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void unCheck(_PageElement pageElement) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException ("Failed to uncheck: '" + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}
	...
	protected String acceptAlertAndReturnConfirmationMessage() {
		try {
			...
			throw new ScriptExecutionException ("failed to accept alert on " + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		}
	}
	...
	protected String dismissAlertAndReturnConfirmationMessage() {
		try {
			...
			throw new ScriptExecutionException ("failed to dismiss alert on " + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		}
	}

	protected void doubleClick(_PageElement pageElement) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException("Failed to doubleclick: '" + "' on " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected String getText(_PageElement pageElement) {
		...
		try {
			...
			throw new ScriptExecutionException("Failed to fetch text: '" + "' of " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
		return text;
	...
	protected String getAttribute(_PageElement pageElement, String attributeName) {
		...
		try {
			...
			throw new ScriptExecutionException("Failed to fetch '" + attributeName + "' of " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	...
	protected boolean isElementDisplayed(_PageElement pageElement) {
		...
		try {
			...
			throw new ScriptExecutionException ("Failed to display: '" + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", e,pageName);
		} finally {
			pageElement = null;
		}
	...
	protected boolean isElementSelected(_PageElement pageElement) {
		boolean isElementDisplayed = false;
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
	...
	protected boolean isElementEnabled(_PageElement pageElement) {
	...
	protected void selectValueFromList(_PageElement pageElement, String value) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException("Failed to Select value: '" + value + "' of " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void mouseOver(_PageElement pageElement) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException("Failed to hover mouse: '" + "' on " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected String getSelectedValueFromList(_PageElement pageElement) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException("Failed to fetch "
					...
					+ " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected ArrayList<String> getAllOptionsInList(_PageElement pageElement) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException("Failed to fetch " + "option Value" + "' of " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}

	protected void bringElementInView (_PageElement pageElement) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
			...
			throw new ScriptExecutionException("Failed to bring Element  " + pageElement.getName() + "  " + " in the current browser view " + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}
	...
	protected void switchToWindow(String windowTitle) throws InterruptedException {
		try{
			...
			throw new ScriptExecutionException ("Failed to search Criteria: ", e,pageName);
		}
	}
	...
	protected void switchToWindow(String windowTitle, String errorTitleWindow) throws InterruptedException 
	{
		try{
			...
			if (alltitles.contains(errorTitleWindow)){
				throw new ScriptExecutionException ("Error Window Encountered While Execution.",pageName);
			}
			else if(alltitles.contains(windowTitle)){
				switchToWindow(windowTitle);

			}else {
				throw new ScriptExecutionException ("Expected page: "+windowTitle+ " NOT FOUND ",pageName);
			}
		}
		catch(Exception e){
			throw new ScriptExecutionException ("Failed to search Criteria: ", e,pageName);
	...
	protected void closeWindow(String windowTitle) throws InterruptedException {
		try {
			...
			throw new ScriptExecutionException("Failed to Close the Window with title \"" + windowTitle + " \"", exception,pageName);
		}
	}
	...
	protected void checkOrUncheck(_PageElement pageElement,Boolean config) {
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
				webElement = getWebElement(pageElement);
	...
	protected String fetchValueFromFields(_PageElement pageElement) {
		String text = new String();
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
	...
	protected String fetchValueFromTextFields(_PageElement pageElement) {
		String text = new String();
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
	...
	protected String fetchValueFromList(_PageElement pageElement){
		String text = new String();
		try {
			WebElement webElement;
			if (!isWebElementAvailableInPageElement(pageElement))
	...
	protected void clearAndEnterDate(_PageElement pageElement,String date)
	{
		try {
			...
			throw new ScriptExecutionException ("Failed to clear and type value: '" + date + "' in " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	...
	protected void switchToFrame(String frameName){
		try {
			...
			throw new ScriptExecutionException ("Failed to switch to Frame: '" + frameName + "' in on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			frameName=null;
		}
	}
	protected void selectDateFromCalender(_PageElement pageElement,String dateToBeAdded) throws InterruptedException{
		String date=null;
	...
	protected void switchForSearchCriteria(_PageElement pageElement,String searchFieldLocator,String searchCriteria,String frameName) throws InterruptedException{
		try {
			click(pageElement);
			Thread.sleep(500);
			switchToWindow();
			_PageElement searchCriteriaTextField=new _PageElement(By.name(searchFieldLocator), "search FieldLocator Textfield", false, WaitType.WAITFORELEMENTTOBEDISPLAYED, 10);
			clearAndSendKeys(searchCriteriaTextField, searchCriteria);
			Thread.sleep(500);
			_PageElement findButton=new _PageElement(By.xpath("//input[@name='"+searchFieldLocator+"']/following::a['firstFocus']"), "Party Code Textfield", false, WaitType.WAITFORELEMENTTOBECLICKABLE, 10);
			click(findButton);
			Thread.sleep(500);
			_PageElement searchCriteriaLink=new _PageElement(By.linkText(searchCriteria), ""+searchCriteria+"Link", false, WaitType.WAITFORELEMENTTOBECLICKABLE, 10);
	...
	protected void switchToWindow() throws InterruptedException {
		...
		try{
			...
			throw new ScriptExecutionException ("Failed to Switch window: ", e,pageName);
		}
	}
	...
	protected void clearAndEnterTime(_PageElement pageElement){
		try {
			RandomCodeGenerator randomCodeGenerator=new RandomCodeGenerator();
			...
			throw new ScriptExecutionException ("Failed to clear and type value: '' in " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}
	public void click(_PageElement pageElement,WebDriver driver){
	...
	protected void bringWebElementInView (_PageElement pageElement) {
		try {
	...
	protected void clearTextField (_PageElement pageElement) {
		try {
			WebElement element;
			if (!isWebElementAvailableInPageElement(pageElement))
				element = getWebElement(pageElement);
			...
			throw new ScriptExecutionException ("Failed to Cleared  in " + pageElement.getName() + " on : '" + pageName + "' Refer to Sheet:  '"+CustomizableEntity.runningSheetName+"' "+ "' Column Name:  '"+CustomizableEntity.runningElement+"' "+ "' Row Reference :  '"+CustomizableEntity.runningRowReference+"' ", exception,pageName);
		} finally {
			pageElement = null;
		}
	}
	protected void clearAndEnterForwardTime(_PageElement pageElement,String timeOfIntimation){
		try {
	...
	protected void switchToMainPage(){
		...
		try{
			...
			throw new ScriptExecutionException ("Failed to Switch"+windowTitle+" window ", e,pageName);
		}
	...
	protected void closeWindow() throws InterruptedException {
		try {
			...
			throw new ScriptExecutionException("Failed to Close the Window  ", exception,pageName);
		}
	}
	...
	protected void clearAndSendAutoPopulateText(_PageElement pageElement,String text){
		try{
